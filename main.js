...
async function loadTemplates() {
    try {
        const response = await fetch('PHP file/get_templates.php');
...
function viewFile(index) {
    ...
    if (file.type.startsWith('image/')) {
        // Show image in modal
        showFileModal(file.name, `<img src="${file.data}" style="max-width:100%; max-height:80vh; border-radius:8px; display:block; margin:auto;">`);
...
function initializeAdminForm() {
    ...
    if (templateForm) {
        templateForm.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch('PHP file/add_template.php', {
                    method: 'POST',
                    headers: {
